Public Class AnalisisTendencias
    ...
    Private Sub btnGenerarGrafica_Click(sender As Object, e As EventArgs) Handles btnGenerarGrafica.Click
        ...
        Dim mes As String = ""
        query2 += " SELECT  " & vbCrLf
        query2 += "     FORMAT(M.Fecha, 'MMMM', 'es-ES') AS Mes, " & vbCrLf
        query2 += "     SUM(CASE WHEN C.ClaseTipoMovimiento = 1 THEN M.Cantidad ELSE 0 END) AS Ingresos, " & vbCrLf
        query2 += "     SUM(CASE WHEN C.ClaseTipoMovimiento = 0 THEN M.Cantidad ELSE 0 END) AS Egresos " & vbCrLf
        query2 += " FROM  " & vbCrLf
        query2 += "     Movimiento M " & vbCrLf
        query2 += " INNER JOIN  " & vbCrLf
        query2 += "     Categoria_Movimiento C  " & vbCrLf
        query2 += "     ON C.ID_Categoria = M.ID_Categoria " & vbCrLf
        query2 += " WHERE  " & vbCrLf
        query2 += "     M.ID_Usuario = '" & usuarioActual & "' " & vbCrLf
        query2 += "     AND M.Fecha BETWEEN '" & fechaInicial & " ' AND '" & fechaFinal & "'  " & vbCrLf
        query2 += " GROUP BY  " & vbCrLf
        query2 += "     FORMAT(M.Fecha, 'MMMM', 'es-ES'), YEAR(M.Fecha), MONTH(M.Fecha) " & vbCrLf
        query2 += " ORDER BY  " & vbCrLf
        query2 += "    MONTH(M.Fecha);  " & vbCrLf

        Conexiones.AbrirConexion()
        Conexiones.Cnn.Open()

        Dim dg As New SqlClient.SqlDataAdapter(query2, Conexiones.Cnn)
        Dim df As New DataSet
        dg.Fill(df)
        If df.Tables(0).Rows.Count > 0 Then
            For i = 0 To df.Tables(0).Rows.Count - 1
                If i = 0 Then
                    mes = "'" + df.Tables(0).Rows(i).Item("Mes") + "'"
                    ingresos = df.Tables(0).Rows(i).Item("Ingresos").ToString
                    egresos = df.Tables(0).Rows(i).Item("Egresos").ToString
                Else
                    mes += ", '" + df.Tables(0).Rows(i).Item("Mes") + "'"
                    ingresos += "," + df.Tables(0).Rows(i).Item("Ingresos").ToString
                    egresos += "," + df.Tables(0).Rows(i).Item("Egresos").ToString
                End If
            Next
            graficaIngresosEgresos2("Egresos vs Ingresos", Me.Page, mes, ingresos, egresos)
        End If
        Conexiones.Cnn.Close()
